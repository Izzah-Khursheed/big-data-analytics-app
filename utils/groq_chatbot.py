...
GROQ_API_URL = st.secrets["https://api.groq.com/openai/v1/chat/completions"]  # Example: "https://api.groq.ai/v1/chat/completions"
GROQ_API_KEY = st.secrets["GROQ_API_KEY"]  # Your API key

def run_groq_chatbot_tab():
    st.header("GroqChat AI Chatbot 🤖")

    if "chat_history" not in st.session_state:
        st.session_state["chat_history"] = []

    user_input = st.text_input("Ask GroqChat anything:")
